...
/**
 * Implementação de um agente X usando o algoritmo Minimax com cortes alfa-beta para um jogo de tabuleiro.
 *
 * @author Davide Pinto, Nuno Carvalho, Robim Rodrigues
 *
 */
public class XAgent {
    public static int expanded_nodes = 0;
    private static final int X_WINS = 100;
    private static final int O_WINS = -100;
    ...
    private static int evaluateLine(Ilayout.ID[] line) {
        ...
        } else if (oCount == Board.N) {            return O_WINS; // O wins
        } else {

            if (xCount > 0 && oCount == 0 && blankCount > 0) {
                return 1; // X has a potential future line
            } else if (xCount == 0 && oCount > 0 && blankCount > 0) {
                return -1; // O has a potential future line
            } else {
